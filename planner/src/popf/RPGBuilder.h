            ...
            for (int i = 0; i < size; ++i) {
                if (fluents[i] == -5) {
                    if (current_analysis->the_problem->metric->opt == VAL::E_MINIMIZE) {
                        toReturn -= weights[i] * Globals::bestSolutionQuality;
                    } else {
                        toReturn += weights[i] * Globals::bestSolutionQuality;
                    }
                    continue;
...
// static vector<list<pair<int, double> > > negativeNumericEffectsToActions;
    ...
    static vector<list<int> > actionsToRPGNumericStartNonExSoEffects;
    static vector<list<int> > actionsToRPGNumericStartExSoEffects;
    static vector<list<int> > actionsToRPGNumericEndNonExSoEffects;
    static vector<list<int> > actionsToRPGNumericEndExSoEffects;
    static list<int> fakeRPGNumericExSoEffects;
    // added here
...
// static vector<pair<int, double> > increasedReset;
    ...
    static vector<FakeTILAction*> timedInitialLiteralsExSoVector;
    ...
    static vector<bool> timewindows;
    static vector<bool> oneshotActions;
    static vector<bool> oneshotTimewindowActions;
    static vector<LiteralSet> propositionTimewindows;
    static vector<set<int> > actionsDependentFromOneshotTimewindows;
...
// static void makeWeightedSum(list<Operand> & formula, pair<list<double>, list<int> > & result);
    ...
    static void findTimewindows();
    static void findOneshotTimewindows(MinimalState & refState);
    static void findDependencyTimewindowsActions();
...
public:
    ...
    static void applyExSoInitialState(vector<double> &values);
    ...
    // External Solver
    static vector<list<int> > & getStartNonExSoEffNumerics() {
        return actionsToRPGNumericStartNonExSoEffects;
    }
    
    static vector<list<int> > & getStartExSoEffNumerics(){
        return actionsToRPGNumericStartExSoEffects;
    }
    static vector<list<int> > & getEndNonExSoEffNumerics() {
        return actionsToRPGNumericEndNonExSoEffects;
    }
    
    static vector<list<int> > & getEndExSoEffNumerics(){
        return actionsToRPGNumericEndExSoEffects;
    }
    
    static list<int> & getFakeExSoEffNumerics(){
        return fakeRPGNumericExSoEffects;
    }
    static set<int> & getSpecialExSo(){
        return specialExSo;
    }
    static set<int> & getDependentExSo(){
        return dependentExSo;
    }
    ...
    /** add a new effect to the numericEffects */

    static void addNumericEff(RPGNumericEffect & numEff){
	rpgNumericEffects.push_back(numEff);
    }

    static void changeNumericEff(RPGNumericEffect & numEff, int position){
        rpgNumericEffects[position]=numEff;
    }
    ...
    static bool isOneshotTimewindow(const int & actID) {
        return oneshotTimewindowActions[actID];
    }
    
    static bool isDependentOnTimewindow(const int &actToPass, const int &idTW){
        set <int> timewindowsactive = actionsDependentFromOneshotTimewindows[actToPass];
        return timewindowsactive.find(idTW)!=timewindowsactive.end();
    }
...
public:
    ...
    int getRelaxedPlan(const MinimalState & theState, const list<StartEvent> * startEventQueue,
                       ...
                       double & finalPlanMakespanEstimate, map<double, list<pair<int, int> > > * justApplied = 0, double tilFrom = 0.001, list<FFEvent>  partialPlan = list<FFEvent>(), bool timewindowsCheck = true, bool isRecalculated = false);
