...
namespace Planner
{
    ...
    bool FF::restartBFSAfterEachNewBestSolution = false;
    ...
    public:
        ...
        void clone(SearchQueueItem* toBeCopied){
            plan = toBeCopied->plan;
            helpfulActions = toBeCopied->helpfulActions;
            heuristicValue = toBeCopied->heuristicValue;
        }
        ...
        void printPlan() {
            if (Globals::globalVerbosity & 2) {
                ...
                for (int i = 0; planItr != planEnd; ++planItr, ++i) {
                    ...
                    if (!planItr->getEffects) cout << " ))";
                    double max = planItr->lpMaxTimestamp;
                    if (max >= DBL_MAX){
                        max = 99999.;
                    }
                    cout << " at " << planItr->lpMinTimestamp << ", " << max;
                    cout << "\n";
                }
            }
        }
    ...
    public:
        ...
        void insert(SearchQueueItem* p, const int category = 1, bool isBackup = false) {
            
            if (FF::costOptimalAStar) {
                const double pCost = (Globals::totalOrder ? p->heuristicValue.makespanEstimate * FF::doubleU  + p->heuristicValue.makespan
                                      : (p->heuristicValue.makespanEstimate - p->heuristicValue.makespan) * FF::doubleU  + p->heuristicValue.makespan);
            ...
            if(isBackup && ModifiedSearch::level==0){
                int totalTIL = RPGBuilder::getTILVec().size();
                prim = totalTIL - p->state()->getInnerState().nextTIL;
            }
            if (FF::WAStar) {
                prim *= FF::doubleU;
                if (FF::timeWAStar) {
    ...
    FF::HTrio FF::calculateHeuristicAndSchedule(ExtendedMinimalState & theState, ExtendedMinimalState * prevState, set<int> & goals, set<int> & goalFluents, ParentData * const incrementalData, list<ActionSegment> & helpfulActions, list<FFEvent> & header, list<FFEvent> & now, const int & stepID, bool considerCache, map<double, list<pair<int, int> > > * justApplied, double tilFrom)
    {
        ...
        list<FFEvent> headerNow = header;
        for(list<FFEvent>::iterator nIt = now.begin();nIt!=now.end();++nIt){
            headerNow.push_back(*nIt);
        }
        //printState(theState);
        ...
        if (considerCache) {
            if (FFcache_upToDate) {
                ...
                double backupFinalPlanMakespanEstimate = makespanEstimate;
                map<double, list<pair<int, int> > >  backupJustApplied;
                if(justApplied){
                    for(map<double,list<pair<int,int> > >::iterator it = justApplied->begin(); it!=justApplied->end();++it){
                        backupJustApplied.insert(make_pair((*it).first,(*it).second));
                    }
                }
                h = RPGBuilder::getHeuristic()->getRelaxedPlan(theState.getInnerState(), &(theState.startEventQueue), minTimestamps, theState.timeStamp,
                                                               ...
                                                               helpfulActions, relaxedPlan, makespanEstimate, justApplied,
                                                               tilFrom,headerNow,TimewindowsChecker::heuristicCheck);
                if(LookAhead::isActive && LookAhead::onlyH){
                    LookAhead::isActive=false;
                    list<ActionSegment> toTrashHA;
                    list<pair<double, list<ActionSegment> > > toTrashRP;
                    list<FFEvent> toTrashH = headerNow;
                    int j = RPGBuilder::getHeuristic()->getRelaxedPlan(theState.getInnerState(), &(theState.startEventQueue), minTimestamps, theState.timeStamp,
                                                                       extrapolatedMin, extrapolatedMax, timeAtWhichValueIsDefined, // for colin-jair heuristic
                                                                       helpfulActions, toTrashRP, backupFinalPlanMakespanEstimate, &backupJustApplied,
                                                                       tilFrom,toTrashH,TimewindowsChecker::heuristicCheck);
                    LookAhead::isActive=true;
                }
                if(LookAhead::isActive && LookAhead::onlyHA){
                    LookAhead::isActive=false;
                    list<ActionSegment> toTrashHA;
                    list<pair<double, list<ActionSegment> > > toTrashRP;
                    list<FFEvent> toTrashH = headerNow;
                    h = RPGBuilder::getHeuristic()->getRelaxedPlan(theState.getInnerState(), &(theState.startEventQueue), minTimestamps, theState.timeStamp,
                                                                   extrapolatedMin, extrapolatedMax, timeAtWhichValueIsDefined,                                  // for colin-jair heuristic
                                                                   toTrashHA, toTrashRP, backupFinalPlanMakespanEstimate, &backupJustApplied, tilFrom,toTrashH,TimewindowsChecker::heuristicCheck);
                    LookAhead::isActive=true;
                }
            ...
            h = RPGBuilder::getHeuristic()->getRelaxedPlan(theState.getInnerState(), &(theState.startEventQueue), minTimestamps, theState.timeStamp,
                                                           extrapolatedMin, extrapolatedMax, timeAtWhichValueIsDefined,                                      // for colin-jair heuristic
                                                           helpfulActions, relaxedPlan, makespanEstimate, justApplied, tilFrom,headerNow,TimewindowsChecker::heuristicCheck);
        ...
        if(stepID > -1){
            if(now.empty()){
                list<FFEvent>::iterator it = header.begin();
                advance(it,stepID);
                nameAction = it->toString();
            }else{
                nameAction = now.begin()->toString();
            }
        }
        
        if (TimewindowsChecker::relaxedCheck){
            bool checkTimewindow =  TimewindowsChecker::checkRelaxedPlanTimeWindows(headerNow,relaxedPlan,theState.getInnerState(),theState.startEventQueue);
            
            if (!checkTimewindow){
                int prevH = h;
                h = RPGBuilder::getHeuristic()->getRelaxedPlan(theState.getInnerState(), &(theState.startEventQueue), minTimestamps, theState.timeStamp,
                                                               extrapolatedMin, extrapolatedMax, timeAtWhichValueIsDefined,                                      // for colin-jair heuristic
                                                               helpfulActions, relaxedPlan, makespanEstimate, justApplied, tilFrom,headerNow,true);
                
                
                if(h>-20) cout << "say something: previous h " <<  prevH << " now " << h << " " <<  nameAction <<  endl;
            }
        }
        
        theState.id = RPGHeuristic::statesEvaluated;
        DotSearchSpace::addNode(theState.id,nameAction, h, theState.idParent);
        if (h < oldBestH) {
            oldBestH = h;
        }
    ...
    FF::HTrio FF::calculateHeuristicAndCompressionSafeSchedule(ExtendedMinimalState & theState, ExtendedMinimalState * prevState, set<int> & goals, set<int> & goalFluents, list<ActionSegment> & helpfulActions, list<FFEvent> & header, list<FFEvent> & now, const int & stepID, map<double, list<pair<int, int> > > * justApplied, double tilFrom)
    {
        ...
        if(LookAhead::isActive && LookAhead::onlyH){
            ...
            list<FFEvent> toTrashH = headerNow;
            int j = RPGBuilder::getHeuristic()->getRelaxedPlan(theState.getInnerState(), &(theState.startEventQueue), minTimestamps, theState.timeStamp,
                                                               theState.getInnerState().secondMin, theState.getInnerState().secondMax, timeAtWhichValueIsDefined,
                                                               helpfulActions, toTrashRP, backupFinalPlanMakespanEstimate, &backupJustApplied,
                                                               tilFrom,toTrashH,TimewindowsChecker::heuristicCheck);
            LookAhead::isActive=true;
        }
        ...
        if (TimewindowsChecker::relaxedCheck){
            ...
            if (!checkTimewindow){
                ...
                if(h>-20) cout << "say something: previous h " <<  prevH << " now " << h << " " <<  RPGHeuristic::statesEvaluated <<  endl;
            }
        }
    ...
    ExtendedMinimalState * FF::applyActionToState(ActionSegment & actionToApply, const ExtendedMinimalState & parent, const list<FFEvent> & plan)
    {
        ...
        const bool localDebug = (Globals::globalVerbosity & 4096);
        if (localDebug) {
        ...
        ExtendedMinimalState * toReturn = 0;
        ...
        //    toReturn->getInnerState().temporalConstraints->printTC();
        return toReturn;
    }
    ...
    void FF::evaluateStateAndUpdatePlan(auto_ptr<SearchQueueItem> & succ, ExtendedMinimalState & state, ExtendedMinimalState * prevState, set<int> & goals, set<int> & goalFluents, ParentData * const incrementalData, list<ActionSegment> & helpfulActionsExport, const ActionSegment & actID, list<FFEvent> & header)
    {
        ...
        if (actID.second == VAL::E_AT_START) {
            ...
            if(DotSearchSpace::writeFluents){
                extraEvent.recordFluents(state.getInnerState());
            }
            ...
            if (!RPGBuilder::getRPGDEs(actID.first->getID()).empty()) { // if it's not a non-temporal action
                ...
                if (DotSearchSpace::writeFluents){
                    extraEventTwo.recordFluents(state.getInnerState());
                }
            ...
            
            {
                ...
                if (DotSearchSpace::writeFluents){
                    pwItr->recordFluents(state.getInnerState());
                }
        ...
        virtual InsertIterator * insertState(ExtendedMinimalState * const e)  __attribute__((warn_unused_result)) {
            ...
            if (toReturn->outerInsertion.second) {
                if (insDebug) {
                    ...
                    void FF::insertStateBackupQueue(auto_ptr<SearchQueueItem>& succ, SearchQueue& backupQueue, bool &WAStart, bool isTIL, auto_ptr<StatesToDelete>& keptStates)
                    {
                        if(ModifiedSearch::isActive && (isTIL || ModifiedSearch::level == 2) ){
                            if(ModifiedSearch::localDebug)
                                cout << "Add TIL to the possible queue" << endl;
                            SearchQueueItem *notToDelete = new SearchQueueItem(succ.get()->state(),false);
                            ExtendedMinimalState * const toHash = succ.get()->state()->clone();
                            keptStates->alsoCleanUp(toHash);
                            notToDelete->clone(succ.get());
                            auto_ptr<SearchQueueItem> toAdd(notToDelete);
                            if (ModifiedSearch::level == 0){
                                backupQueue.insert(toAdd.release(),1,true);
                            }else{
                                WAStart = true;
                                backupQueue.insert(toAdd.release(),1,true);
                                WAStart = false;
                    ...
                    Solution FF::searchBFS(bool &runBestFirstSearch, SearchQueue &searchQueue, set<int>& goals, auto_ptr<StateHash>& visitedStates,set<int>& numericGoals, auto_ptr<StatesToDelete>& statesKept, bool &reachedGoal, HTrio& bestHeuristic, SearchQueue & oldSearchQueue,SearchQueue &backupQueue, bool separateSearch, double whenToStop, int bestH)
                    {
                        
                        while (!runBestFirstSearch && !searchQueue.empty()) {
                            ...
                            if(separateSearch){
                                /*
                                 if (currSQI->heuristicValue.heuristicValue < bestH){
                                 oldSearchQueue.insert(currSQI.release(), 1);
                                 break;
                                 }
                                 */
                                //	    cout << "when to stop " << whenToStop << endl;
                                //return workingBestSolution;
                                if(!currSQI->plan.empty()){
                                    list<FFEvent>::iterator lastAction = currSQI->plan.end();
                                    --lastAction;
                                    while(!lastAction->getEffects){
                                        --lastAction;
                                    }
                                    //change this... compression safe
                                    bool skip = true;
                                    if(lastAction->time_spec == VAL::E_AT_END){
                                        bool isCompressionSafe = TemporalAnalysis::canSkipToEnd(lastAction->action->getID());
                                        //		    cout << *(lastAction->action) << " is compression safe " << isCompressionSafe << endl;
                                        if (isCompressionSafe)
                                            skip = false;
                                    }
                                    if(lastAction->lpMinTimestamp > (whenToStop) && skip /*&& lastAction->time_spec != VAL::E_AT_END*/ ){
                                        //                       cout << "Skip because is too late" << endl;
                                        continue;
                                    }
                                    //                  cout <<  lastAction->lpMinTimestamp << " " << whenToStop << endl;
                                    if (lastAction->time_spec == VAL::E_AT && lastAction->lpMinTimestamp==whenToStop ){
                                        //                     cout <<" *** IT'S TIME TO STOP" << endl;
                                        SearchQueueItem *notToDelete = new SearchQueueItem(currSQI.get()->state(),false);
                                        ExtendedMinimalState * const toHash = currSQI.get()->state()->clone();
                                        notToDelete->clone(currSQI.get());
                                        auto_ptr<SearchQueueItem> toAdd(notToDelete);
                                        //backupQueue.insert(toAdd.release(),1);
                                        oldSearchQueue.insert(currSQI.release(), 1);
                                        break;
                                        ...
                                        if (succ->heuristicValue.heuristicValue != -1.0) {
                                            ...
                                            if (succ->heuristicValue.heuristicValue == 0.0) {
                                                ...
                                                if (improvedOnBest && FF::restartBFSAfterEachNewBestSolution) {
                                                    cout << "Restarting best-first search\n";
...
#endif
                                            // changed this because also if the state is a goal state, from this we can have better solutions
                                            /*if (succ->heuristicValue.heuristicValue != 0.0)*/ {
                    ...
                    Solution FF::search(bool & reachedGoal)
                    {
                        ...
                        
                        {
                            ...
                            //	RPGBuilder::applyExSoInitialState(tinitialFluents);
                        ...
                        
                        {
                            ...
                            if(ModifiedSearch::isActive){
                                SearchQueueItem *notToDelete = new SearchQueueItem(initialSQI->state(),false);
                                ExtendedMinimalState * const toHash = initialSQI->state()->clone();
                                //statesKept->alsoCleanUp(toHash);
                                notToDelete->clone(initialSQI);
                                auto_ptr<SearchQueueItem> toAdd(notToDelete);
                                if(ModifiedSearch::level==0){
                                    backupQueue.insert(toAdd.release(),1,true);
                                }else{
                        ...
                        while (!searchQueue.empty()) {
                            ...
                            if(currSQI->state()->getInnerState().nextTIL<RPGBuilder::getTILVec().size()){
                                RPGBuilder::FakeTILAction * const tilItr = RPGBuilder::getTILVec()[currSQI->state()->getInnerState().nextTIL];
                                double stopnow = tilItr->duration;
                                if(stopnow>whenToStopBackup){
                                    whenToStopBackup = stopnow;
                                }
                            }
                            ...
                            for (; helpfulActsItr != helpfulActsEnd; ++helpfulActsItr) {
                                ...
                                if (!tsSound) {
                                    if (Globals::globalVerbosity & 2) {
                                        cout << "\tTemporally invalid choice\n";
                                    } else if (Globals::globalVerbosity & 1) cout << "t"; cout.flush();
                                } else {
                                    ...
                                    if (visitTheState) {
                                        ...
                                        if (succ->heuristicValue.heuristicValue != -1.0) {
                                            ...
                                            if ((succ->heuristicValue.heuristicValue < bestHeuristic.heuristicValue)
                                                ...
                                                bestHeuristic = succ->heuristicValue;
                                                cout << "b (" << bestHeuristic.heuristicValue << " | " << bestHeuristic.makespan << ")" ; cout.flush();
                                                insertStateBackupQueue(succ,backupQueue,WAStar,isTIL,statesKept);
                                                searchQueue.clear();
                            ...
                            if(ModifiedSearch::isActive && searchQueue.empty()){
                                if(ModifiedSearch::localDebug){
                                    cout << "Resorting to best-first search from the latest til " << whenToStopBackup << endl;
                                }
                                if(!backupQueue.empty()){
                                    double whenToStop = whenToStopBackup;
                                    auto_ptr<StateHash> visitedStatesForMix(getStateHash());
                                    if(ModifiedSearch::level==0){
                                        while(!backupQueue.empty()){
                                            SearchQueueItem* currSQI = backupQueue.pop_front();			
                                            SearchQueue newQueue;
                                            newQueue.insert(currSQI,1);
                                            bool runBestFirstSearch = false;
                                            Solution partial = searchBFS(runBestFirstSearch,newQueue,goals,visitedStatesForMix,numericGoals,statesKept,reachedGoal,bestHeuristic,searchQueue, backupQueue, ModifiedSearch::isActive,whenToStop,bestHeuristic.heuristicValue);
                                        ...
                                        bool runBestFirstSearch = false;
                                        Solution partial = searchBFS(runBestFirstSearch,backupQueue,goals,visitedStatesForMix,numericGoals,statesKept,reachedGoal,bestHeuristic,searchQueue, backupQueue, ModifiedSearch::isActive,whenToStop,bestHeuristic.heuristicValue);
                        ...
                        if(!skipEHC){
                            DotSearchSpace::changeSearch = RPGHeuristic::statesEvaluated;
                        }
                        bool runBestFirstSearch = true;
                        
                        while (runBestFirstSearch) {
                            ...
                            Solution partial = searchBFS(runBestFirstSearch,searchQueue,goals,visitedStates,numericGoals,statesKept,reachedGoal,bestHeuristic,searchQueue, backupQueue);
                            
                            while (!searchQueue.empty()) {
                                delete searchQueue.pop_front();
                            }                
