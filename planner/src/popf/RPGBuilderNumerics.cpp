...
double RPGBuilder::ArtificialVariable::evaluateWCalculate(const vector<double> & fluentTable, const int & pneCount)
{
    ...
    for (int i = 0; i < size; ++i) {
        ...
        double w = weights[i];
        if (var == -5) {            
            if (current_analysis->the_problem->metric->opt == VAL::E_MINIMIZE) {
                toReturn -= w * Globals::bestSolutionQuality;
            } else {
                toReturn += w * Globals::bestSolutionQuality;
            }
            continue;
...
bool RPGBuilder::RPGNumericPrecondition::isSatisfiedWCalculate(const vector<double> & maxFluents) const
{
    ...
    static double workingSolutionQuality;
    int var = LHSVariable;
    
    if (var == -5) {
        workingSolutionQuality = Globals::bestSolutionQuality;
        if (current_analysis->the_problem->metric->opt == VAL::E_MINIMIZE && workingSolutionQuality != 0.0) {
            workingSolutionQuality = -workingSolutionQuality;
        }
        if (op == VAL::E_GREATER) {
            return (workingSolutionQuality > RHSConstant);
        } else {
            return (workingSolutionQuality >= RHSConstant);
        }
    } else if (var == -21) {
        workingSolutionQuality = Globals::bestSolutionQuality;
        if (current_analysis->the_problem->metric->opt == VAL::E_MAXIMIZE && workingSolutionQuality != 0.0) {
            workingSolutionQuality = -workingSolutionQuality;
        }
        if (op == VAL::E_GREATER) {
            return (workingSolutionQuality > RHSConstant);
    ...
    if (var < pneCount) {        
        ...
        if (op == VAL::E_GREATER) {
            return (LHSConstant * maxFluents[var] > RHSConstant);
        } else {
            return (LHSConstant * maxFluents[var] >= RHSConstant);
        }
    }
    ...
    if (var < pneCount) {
        ...
        if (op == VAL::E_GREATER) {
            return (LHSConstant * localVal > RHSConstant);
        } else {
            return (LHSConstant * localVal >= RHSConstant);
        }
    }
        ...
        op(opIn), valid(true), polarity(p)
{
    ...
    if (RHSformulaIn) {
        ...
        RHSformula.push_back(RPGBuilder::Operand((int) -5));
    }
...
//  const double rhsBefore = rhs;
	...
	if(debug) cout << "Looked up " << lookup->first << " -> " << lookup->second << "\n";
        if (lookup != lhs.end()) {
            if (debug) {
		...
		{
		    localMap.insert(make_pair(currEff.fluentIndex, lookup->second));
		}
		else
		{
		    discounted[currEff.fluentIndex] = lookup->second; // Only remove the variable if
		}
    ...
    if (ctsEffs) {
        ...
        for (int v = 0; v < ctsVarCount; ++v) {
            ...
            if (lookup != lhs.end()) {
                ...
                unchanged = false;
                //lhs.erase(lookup); // Don't do this - variables are eliminated from lhs only for assignments, via the discount method.
            }
        }
    }
	...
	lhs[moItr->first] -= discounted[moItr->first];  // Discount for assignments (use this approach because otherwise we end up removing variables that appear elsewhere
	if(fabs(lhs[moItr->first]) < 0.00000001) lhs.erase(moItr->first);
	unchanged = false;

	if (debug && moItr->second.second != 0.0) {
	    cout << "* RHS is now " << rhs << " following " << moItr->second.second << " adjustment\n";
        ...
        for (; mergeItr != mergeEnd; ++mergeItr) {
            ...
            if (debug) {
		cout << "* Changes coeffienct on " << *(getPNE(mergeItr->first)) << " by " << mergeItr->second << " to " << dest->second << endl;
            }
	    unchanged = false;
	    if (fabs(dest->second) < 0.00000000001)
	    {
		if(debug) cout << "Erasing variable " << *(getPNE(mergeItr->first)) << "\n";
		lhs.erase(dest);
	    }
        }
    }
    ...
    if (lhs.empty()) { //variable-less precondition
        if(debug)
	{
	    cout << "No variables in precondition! " << rhs << "\n";
	}
        ...
        if (!liItr->second && liItr->first == -1) {
            ...
            if (liItr->second) {
                ...
                if (liItr->first >= 0) {
                    ...
                    if (liItr->first == -5) {
                        cout << "<current best quality>";
                    } else if (liItr->first == -21) {
                        cout << "-<current best quality>";
                    } else if (liItr->first == -3) {
                        cout << "?duration";
                                      ...
                                      vector<double> & localMaxNeed, const int & i, const VAL::time_spec & passTimeSpec)
{
    ...
    for (; cpItr != cpEnd; ++cpItr) {
        ...
        {
            bool containsSolutionQualityVar = false;
            bool containsSpecialVars = false;
            bool containsVars = false;
            
            for (int pass = 0; pass < 2; ++pass) {
                const list<int> & varList = (pass ? RHSvariable.second : LHSvariable.second);
                list<int>::const_iterator vItr = varList.begin();
                const list<int>::const_iterator vEnd = varList.end();
                ...
                for (; vItr != vEnd; ++vItr) {
                    ...
                    if (*vItr == -5 || *vItr == -21) {
                        containsSolutionQualityVar = true;
                        continue;
                    }            
                    
                    if (*vItr < 0) {
                        containsSpecialVars = true;
                        continue;
                    }
...
void RPGBuilder::buildRPGNumericPreconditions()
{
            ...
            for (int ceID = 0; !contradictoryPreconditions && ceItr != ceEnd; ++ceItr, ++prcItr, ++ceID) {
                ...
                for (int pass = 0; pass < 3; ++pass) {
                    ...
                    for (; destItr != destEnd; ++destItr) {
                        ...
                        assert(durative || (passTimeSpec == VAL::E_AT_START));                        
                    }
                }
            }
        }
    ...
    if (Globals::optimiseSolutionQuality && current_analysis->the_problem->metric) {
        
        const VAL::comparison_op metricOp = (current_analysis->the_problem->metric->opt == VAL::E_MAXIMIZE ? VAL::E_GREATER : VAL::E_LESS);
        
        list<NumericPrecondition> dummy;
        
        dummy.push_back(NumericPrecondition(metricOp, const_cast<VAL::expression*>(current_analysis->the_problem->metric->expr), 0, 0, theTC, true));
        
        int unsat = 0;
        list<int> dest;
            
        if (processPreconditions(artificialVariableSet, rpgNumericPreconditionSet, dummy, dest, unsat, negOffset, offset, precCount, avCount, localMaxNeed, -1, VAL::E_AT)) {
            
            if (dest.empty()) {
                cout << "Warning: not injecting a global extra goal for solution quality, either because it's constant valued, or refers to makespan (which is currently unsupported).\n";
            } else {
                assert(dest.size() == 1);
                numericRPGGoals.push_back(pair<int, int>(dest.front(), -1));
                rpgNumericGoalDeadlines.push_back(DBL_MAX);
                
            }
        } else {
            cout << "Problem cannot be optimised: the metric cannot be represented as a weighted sum of variables\n";
            exit(1);
        }
    ...
    numericAchievedInLayer = vector<EpsilonResolutionTimestamp>(precCount,EpsilonResolutionTimestamp::undefined());
    numericAchievedInLayerReset = vector<EpsilonResolutionTimestamp>(precCount,EpsilonResolutionTimestamp::undefined());
    numericAchievedBy = vector<ActionFluentModification*>(precCount,(ActionFluentModification*) 0);
    numericAchievedByReset = vector<ActionFluentModification*>(precCount,(ActionFluentModification*) 0);
...
void RPGBuilder::buildRPGNumericEffects()
{
    ...
    actionsToRPGNumericStartNonExSoEffects = vector<list<int> >(opCount);
    actionsToRPGNumericStartExSoEffects = vector<list<int> >(opCount);
    actionsToRPGNumericEndNonExSoEffects = vector<list<int> >(opCount);
    actionsToRPGNumericEndExSoEffects = vector<list<int> >(opCount);
    actionsToRPGNumericTilEffects = vector<list<int> > (timedInitialLiteralsVector.size());
                        ...
                        for (; dlItr != dlEnd; ++dlItr, ++ilItr) {
                            if (*dlItr < 0.0) { // for negative weights
                                if (*ilItr == -3) {
                                    ...
                                    *ilItr += negOffset;
                                    *dlItr = 0.0 - *dlItr;
                                }else if (fabs(*dlItr) < 0.000000001)
				{
				    *dlItr = 0;
				    ++rems;
				}
                    ...
                    int lSize = weightedSum.first.size() - rems;
                        ...
                        } else if(*wItr != 0)
			{
                    ...
                    if (doingMainEffects) {
                        ...
                        if(ExternalSolver::isActive){
                            std::string effectString =  (getPNE(currEffect.fluentIndex)->getHead()->getName());
                            list<string> parameters = ExternalSolver::externalSolver->getParameters();
                            if (std::find(parameters.begin(),parameters.end(),effectString)!=parameters.end()){
                                (pass ? actionsToRPGNumericEndExSoEffects : actionsToRPGNumericStartExSoEffects)[act].push_back(insItr.first->first.ID);
                            }else{
                                (pass ? actionsToRPGNumericEndNonExSoEffects : actionsToRPGNumericStartNonExSoEffects)[act].push_back(insItr.first->first.ID);
                            }
                        }
                    } else {
                        (*prcItr).addNumericEffect(insItr.first->first.ID, (pass ? VAL::E_AT_END : VAL::E_AT_START), act);
                    }
...

    {
        ...
        for (int tilID = 0; tilID < tilCount; ++tilID) {
            ...
            map<int,double>::const_iterator neItr = timedInitialLiteralsVector[tilID]->numEffects.begin();
            const map<int,double>::const_iterator neEnd = timedInitialLiteralsVector[tilID]->numEffects.end();
            
            for (; neItr != neEnd; ++neItr) {
                
                static const vector<double> wVector;
                static const vector<int> vVector;

                RPGNumericEffect rpgEffect(effID, neItr->first, true, wVector, vVector, 0, neItr->second);
                ...
                if (localDebug) {
                    cout << "TIL " << tilID << " has effect:\n";
                    cout << "\t" << *(getPNE(neItr->first)) << "= " << neItr->second;                        
                    cout << "\n";
                    if (insItr.second) cout << "Effect hasn't been seen before anywhere\n";
                }
                
                insItr.first->second.push_back(pair<int, VAL::time_spec>(tilID, VAL::E_AT));
                timedInitialLiteralsVector[tilID]->rpgNumericEffects.push_back(insItr.first->first.ID);
                actionsToRPGNumericTilEffects[tilID].push_back(insItr.first->first.ID);
            }
        }
        ...
        if(ExternalSolver::isActive){
            for (int v = 0; v<RPGBuilder::getPNECount() ; v++){
                list<string> parameters = ExternalSolver::externalSolver->getParameters();
                list<string> special = ExternalSolver::externalSolver->getDependencies();
                ...
                specialExSo.insert(v);

                static const vector<double> wVector;
                static const vector<int> vVector;
                
                RPGNumericEffect rpgEffect(effID, v, true, wVector, vVector, 0, -3.14);
                pair<map<RPGNumericEffect, list<pair<int, VAL::time_spec> > >::iterator, bool> insItr
                = rpgNumericEffectSet.insert(pair<RPGNumericEffect, list<pair<int, VAL::time_spec> > >(rpgEffect, emptyList));
                ...
                fakeRPGNumericExSoEffects.push_back(insItr.first->first.ID);
            } // end add fake ipsa effects ?
        }
    }
...
void RPGBuilder::buildMetric(VAL::metric_spec * ms)
{
    ...
    if (forceMakespanMetric) {
        theMetric = new Metric(true);
        theMetric->weights.push_back(1.0);
        theMetric->variables.push_back(-4);
    } else {
