...
void usage(char * argv[])
{
    ...
    cout << "\t" << "-I" << "\t\t" << "Disable the hybrid Bellman-Ford--LP solver\n";
#ifndef BUILDINGCOLIN
...
int main(int argc, char * argv[])
{
    ...
    while (argcount < argc && argv[argcount][0] == '-') {
        ...
        if (remainder == "citation") {
            ...
            switch (argv[argcount][1]) {
                ...
                case '\\': {
                    RPGBuilder::forceMakespanMetric = true;
                    break;
                }
                case 'G': {
                    FF::biasG = true;
                ...
                case 't': {
                    if (argv[argcount][2] == 0) {
                        cerr << "Error: must specify a time limit after -t, e.g. -t1800\n";
                        usage(argv);
                        exit(1);
                    }
                ...
                case 'u' : {
                    ModifiedSearch::isActive = true;
                    if (argv[argcount][2] != 0)
                        ModifiedSearch::level = atoi(&(argv[argcount][2]));
                    else
                        ModifiedSearch::level = 0;
                    break;
                }
                case 'w': {
                    LookAhead::isActive = true;
                    if (argv[argcount][2] != 0) {
                        LookAhead::nLook = atoi(&(argv[argcount][2]));
                    }else{
                        LookAhead::nLook = 0;
                        LookAhead::isActive = false;
                        cout << "No # of step ahead specified, set = 0\n";
                    }
                    break;
                }
                ...
                case 'A' : {
                    if (argv[argcount][2] != 0){
                        if (atoi(&(argv[argcount][2])) == 1){
                            LookAhead::onlyH = true;
                        }
                        if(atoi(&(argv[argcount][2])) == 2){
                            LookAhead::onlyHA = true;
                        }
                    }
                    break;
                }
                case 'x' : {
                    ...
                    if (argv[argcount][2] != 0) {
                        if(argv[argcount][2]=='x'){
                            ExternalSolver::heuristicActive = true;
                            ExternalSolver::timeToCalculate=false;
                        }else{
                            ExternalSolver::heuristicActive = false;
                            ExternalSolver::timeToCalculate=false;
                        }
                    }
                    break;
                ...
                case 'Y' : {
                    if (argv[argcount][2] != 0) {
                        int index = atoi(&(argv[argcount][2]));
                        if (index == 1){
                            TimewindowsChecker::heuristicCheck = true;
                        }else if(index == 2){
                            TimewindowsChecker::relaxedCheck = true;
                        }
                    }
                    break;
                }
                ...
                case 'N': {
                    Globals::optimiseSolutionQuality = true;
                    FF::restartBFSAfterEachNewBestSolution = true;
                    if (argv[argcount][2] != 0) {
                        Globals::givenSolutionQualityDefined = true;
                        Globals::givenSolutionQuality = atof(&(argv[argcount][2]));
                    }
    ...
    if(ExternalSolver::isActive){
        if(argc<argcount+3){
            cerr << "error in the format.. with external solver" << endl;
            return 0;
            
        }
        ExternalSolver::name = argv[argcount+2];
        ExternalSolver::verbose = (Globals::globalVerbosity & 2);
        string input[argc-argcount-3];
        int nInput = argc-argcount -3;
        for(int i = 0; i<argc - argcount-3;++i){
            input[i] = argv[argcount+3+i];
            //cout <<"arguments : "<< i << " " << argv[argcount+3+i] << endl;
        }
        dlerror();
        handle = dlopen(ExternalSolver::name.c_str(), RTLD_LAZY);
        if ((dlerror()) != NULL) {
            /* handle error, the symbol wasn't found */
            cerr <<"library : " << ExternalSolver::name << " not found!" << endl;
            return 0;
        }else{
            ExternalSolver *(*create)();
            create = (ExternalSolver* (*)())dlsym(handle,"create_object");
            ExternalSolver::externalSolver = (ExternalSolver*)create();
            ExternalSolver::externalSolver->loadSolver(input,nInput);
            list<string> variables = ExternalSolver::externalSolver->getDependencies();
        }
    }
#ifdef STOCHASTICDURATIONS
    ...
    if (spSoln) {
        ...
        for (int pass = 0; pass < 2; ++pass) {
            if (pass) {
                ...
                if(ExternalSolver::isActive){
                    cout << "; External Solver: " << double(ExternalSolver::time) << endl;
                }
                if(writeSearch){
            ...
            if (writeFluents){
                FFEvent::printVariables(*spSoln,nameOuputFluents);
                cout << "\n" << nameOuputFluents << " written" << endl;
            }
        }
